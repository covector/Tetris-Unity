...
public class Spin_Control_Zpiece : MonoBehaviour
{
    ...
    void Update()
    {
        if (!(landed | FindObjectOfType<Master_Control>().paused))
        {
            ...
            {
                bool holdbk = FindObjectOfType<Master_Control>().Holdblock("Z");
                if (holdbk)
                {
                    Destroy(gameObject);
                }
            ...
            {
                float final = FindObjectOfType<Master_Control>().InstaDrop(state.position.y, state.position.x, "Z", (int)(state.eulerAngles.z));
            ...
            if (((Input.GetKeyDown(clockhotkey) & !(Input.GetKeyDown(anticlockhotkey))) | (Input.GetKeyDown(anticlockhotkey) & !(Input.GetKeyDown(clockhotkey)))) & (FindObjectOfType<Master_Control>().Intersect("Z", state.position.y, state.position.x, (int)state.eulerAngles.z)))
            {
    ...
    bool Bump_wall_L()
    {
        ...
        bool left_block = FindObjectOfType<Master_Control>().CheckCoord("Z", state.position.y, state.position.x, state.eulerAngles.z, "Left") | (FindObjectOfType<Master_Control>().CheckCoord("Z", state.position.y, state.position.x, state.eulerAngles.z, "LDiag") & !(FindObjectOfType<Master_Control>().CheckCoord("Z", state.position.y, state.position.x, state.eulerAngles.z, "Down")) & div(dropped_time + Time.deltaTime, 1 / speed) > dropped_unit);
    ...
    bool Bump_wall_R()
    {
        ...
        bool right_block = FindObjectOfType<Master_Control>().CheckCoord("Z", state.position.y, state.position.x, state.eulerAngles.z, "Right") | (FindObjectOfType<Master_Control>().CheckCoord("Z", state.position.y, state.position.x, state.eulerAngles.z, "RDiag") & !(FindObjectOfType<Master_Control>().CheckCoord("Z", state.position.y, state.position.x, state.eulerAngles.z, "Down")) & div(dropped_time + Time.deltaTime, 1 / speed) > dropped_unit);
    ...
    bool Bump_ground()
    {
        ...
        bool blocks = FindObjectOfType<Master_Control>().CheckCoord("Z", state.position.y, state.position.x, state.eulerAngles.z, "Down");
    ...
    void Dead_block()
    {
        FindObjectOfType<Master_Control>().AddToBoard(state.position.y, state.position.x, (int)(state.eulerAngles.z), "Z");
    ...
    void StateChange()
    {
        float ghoststate = FindObjectOfType<Master_Control>().ChangeGhostPiece("Z", state.position.y, state.position.x, (int)state.eulerAngles.z);
