...
public class Spin_Control_Lpiece : MonoBehaviour
{
    ...
    void Update()
    {
        if (!(landed | FindObjectOfType<Master_Control>().paused))
        {
            ...
            {
                bool holdbk = FindObjectOfType<Master_Control>().Holdblock("L");
                if (holdbk)
                {
                    Destroy(gameObject);
                }
            ...
            #region Spin Control
            // Clockwise spin
            if (Input.GetKeyDown(clockhotkey) & !(Input.GetKeyDown(anticlockhotkey)) & FindObjectOfType<Master_Control>().RotationChoice("L", state.position.y, state.position.x, (int)state.eulerAngles.z, -90, hold, moving_already) != null)
            {
                soundFX[5].Play();
                float[] deltapos = FindObjectOfType<Master_Control>().RotationChoice("L", state.position.y, state.position.x, (int)state.eulerAngles.z, -90, hold, moving_already);
                state.eulerAngles = new Vector3(0, 0, (state.eulerAngles.z - 90) % 360);
                state.position += new Vector3(deltapos[1], deltapos[0], 0);
                if (ghost) { StateChange(); }
            }
            // Anticlockwise spin
            if (Input.GetKeyDown(anticlockhotkey) & !(Input.GetKeyDown(clockhotkey)) & FindObjectOfType<Master_Control>().RotationChoice("L", state.position.y, state.position.x, (int)state.eulerAngles.z, 90, hold, moving_already) != null)
            {
                soundFX[5].Play();
                float[] deltapos = FindObjectOfType<Master_Control>().RotationChoice("L", state.position.y, state.position.x, (int)state.eulerAngles.z, 90, hold, moving_already);
                state.eulerAngles = new Vector3(0, 0, (state.eulerAngles.z + 90) % 360);
            ...
            {
                float final = FindObjectOfType<Master_Control>().InstaDrop(state.position.y, state.position.x, "L", (int)(state.eulerAngles.z));
            ...
            if (((Input.GetKeyDown(clockhotkey) & !(Input.GetKeyDown(anticlockhotkey))) | (Input.GetKeyDown(anticlockhotkey) & !(Input.GetKeyDown(clockhotkey)))) & (FindObjectOfType<Master_Control>().Intersect("L", state.position.y, state.position.x, (int)state.eulerAngles.z)))
            {
    ...
    bool Bump_wall_L()
    {
        ...
        bool left_block = FindObjectOfType<Master_Control>().CheckCoord("L", state.position.y, state.position.x, state.eulerAngles.z, "Left") | (FindObjectOfType<Master_Control>().CheckCoord("L", state.position.y, state.position.x, state.eulerAngles.z, "LDiag") & !(FindObjectOfType<Master_Control>().CheckCoord("L", state.position.y, state.position.x, state.eulerAngles.z, "Down")) & div(dropped_time + Time.deltaTime, 1 / speed) > dropped_unit);
    ...
    bool Bump_wall_R()
    {
        ...
        bool right_block = FindObjectOfType<Master_Control>().CheckCoord("L", state.position.y, state.position.x, state.eulerAngles.z, "Right") | (FindObjectOfType<Master_Control>().CheckCoord("L", state.position.y, state.position.x, state.eulerAngles.z, "RDiag") & !(FindObjectOfType<Master_Control>().CheckCoord("L", state.position.y, state.position.x, state.eulerAngles.z, "Down")) & div(dropped_time + Time.deltaTime, 1 / speed) > dropped_unit);
    ...
    bool Bump_ground()
    {
        ...
        bool blocks = FindObjectOfType<Master_Control>().CheckCoord("L", state.position.y, state.position.x, state.eulerAngles.z, "Down");
    ...
    void Dead_block()
    {
        FindObjectOfType<Master_Control>().AddToBoard(state.position.y, state.position.x, (int)(state.eulerAngles.z), "L");
    ...
    void StateChange()
    {
        float ghoststate = FindObjectOfType<Master_Control>().ChangeGhostPiece("L", state.position.y, state.position.x, (int)state.eulerAngles.z);
