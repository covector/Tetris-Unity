...
public class Spin_Control_Ipiece : MonoBehaviour
{
    ...
    void Update()
    {
        if (!(landed | FindObjectOfType<Master_Control>().paused))
        {
            ...
            {
                bool holdbk = FindObjectOfType<Master_Control>().Holdblock("I");
                if (holdbk)
                {
                    Destroy(gameObject);
                }
            ...
            #region Spin Control
            // Clockwise spin
            if (Input.GetKeyDown(clockhotkey) & !(Input.GetKeyDown(anticlockhotkey)) & FindObjectOfType<Master_Control>().RotationChoice("I", state.position.y, state.position.x, (int)state.eulerAngles.z, -90, hold, moving_already) != null)
            {
                soundFX[5].Play();
                float[] deltapos = FindObjectOfType<Master_Control>().RotationChoice("I", state.position.y, state.position.x, (int)state.eulerAngles.z, -90, hold, moving_already);
                state.eulerAngles = new Vector3(0, 0, (state.eulerAngles.z - 90) % 360);
                state.position += new Vector3(deltapos[1], deltapos[0], 0);
                if (ghost) { StateChange(); }
            }
            // Anticlockwise spin
            if (Input.GetKeyDown(anticlockhotkey) & !(Input.GetKeyDown(clockhotkey)) & FindObjectOfType<Master_Control>().RotationChoice("I", state.position.y, state.position.x, (int)state.eulerAngles.z, 90, hold, moving_already) != null)
            {
                soundFX[5].Play();
                float[] deltapos = FindObjectOfType<Master_Control>().RotationChoice("I", state.position.y, state.position.x, (int)state.eulerAngles.z, 90, hold, moving_already);
                state.eulerAngles = new Vector3(0, 0, (state.eulerAngles.z + 90) % 360);
            ...
            {
                float final = FindObjectOfType<Master_Control>().InstaDrop(state.position.y, state.position.x, "I", (int)(state.eulerAngles.z));
            ...
            if (((Input.GetKeyDown(clockhotkey) & !(Input.GetKeyDown(anticlockhotkey))) | (Input.GetKeyDown(anticlockhotkey) & !(Input.GetKeyDown(clockhotkey)))) & (FindObjectOfType<Master_Control>().Intersect("I", state.position.y, state.position.x, (int)state.eulerAngles.z)))
            {
    ...
    bool Bump_wall_L()
    {
        ...
        bool left_wall_vert1 = (Comp_float(state.eulerAngles.z, 90f) & Comp_float(state.position.x, -1.6f)) | (Comp_float(state.eulerAngles.z, 90f) & (clockwise | anticlockwise) & Comp_float(state.position.x, -1.2f));
        bool left_wall_vert2 = (Comp_float(state.eulerAngles.z, 270f) & Comp_float(state.position.x, -2f)) | (Comp_float(state.eulerAngles.z, 270f) & (clockwise | anticlockwise) & (Comp_float(state.position.x, -1.6f) | Comp_float(state.position.x, -1.2f)));
        bool left_wall_hori = (Comp_float(state.eulerAngles.z, 0f) | Comp_float(state.eulerAngles.z, 180f)) & !(anticlockwise | clockwise) & Comp_float(state.position.x, -1.2f);
        bool left_block = FindObjectOfType<Master_Control>().CheckCoord("I", state.position.y, state.position.x, state.eulerAngles.z, "Left") | (FindObjectOfType<Master_Control>().CheckCoord("I", state.position.y, state.position.x, state.eulerAngles.z, "LDiag") & !(FindObjectOfType<Master_Control>().CheckCoord("I", state.position.y, state.position.x, state.eulerAngles.z, "Down")) & div(dropped_time + Time.deltaTime, 1 / speed) > dropped_unit);
        bool left_wall = left_wall_vert1 | left_wall_vert2 | left_wall_hori | left_block;
    ...
    bool Bump_wall_R()
    {
        ...
        bool right_wall_vert1 = (Comp_float(state.eulerAngles.z, 270f) & Comp_float(state.position.x, 1.6f)) | (Comp_float(state.eulerAngles.z, 270f) & (clockwise | anticlockwise) & Comp_float(state.position.x, 1.2f));
        bool right_wall_vert2 = (Comp_float(state.eulerAngles.z, 90f) & Comp_float(state.position.x, 2f)) | (Comp_float(state.eulerAngles.z, 90f) & (clockwise | anticlockwise) & (Comp_float(state.position.x, 1.6f) | Comp_float(state.position.x, 1.2f)));
        bool right_wall_hori = (Comp_float(state.eulerAngles.z, 0f) | Comp_float(state.eulerAngles.z, 180f)) & !(anticlockwise | clockwise) & Comp_float(state.position.x, 1.2f);
        bool right_block = FindObjectOfType<Master_Control>().CheckCoord("I", state.position.y, state.position.x, state.eulerAngles.z, "Right") | (FindObjectOfType<Master_Control>().CheckCoord("I", state.position.y, state.position.x, state.eulerAngles.z, "RDiag") & !(FindObjectOfType<Master_Control>().CheckCoord("I", state.position.y, state.position.x, state.eulerAngles.z, "Down")) & div(dropped_time + Time.deltaTime, 1 / speed) > dropped_unit);
        bool right_wall = right_wall_hori | right_wall_vert1 | right_wall_vert2 | right_block;
    ...
    bool Bump_ground()
    {
        ...
        bool vert = (Comp_float(state.position.y, -3.6f) & ((Comp_float(state.eulerAngles.z, 270f) | Comp_float(state.eulerAngles.z, 90f)) & !(clockwise | anticlockwise)));
        bool hori1 = Comp_float(state.eulerAngles.z, 0f) & (Comp_float(state.position.y, -4.4f) | ((Comp_float(state.position.y, -3.6f) | Comp_float(state.position.y, -4f)) & (clockwise | anticlockwise)));
        bool hori2 = Comp_float(state.eulerAngles.z, 180f) & (Comp_float(state.position.y, -4f) | (Comp_float(state.position.y, -3.6f) & (clockwise | anticlockwise)));
        bool blocks = FindObjectOfType<Master_Control>().CheckCoord("I", state.position.y, state.position.x, state.eulerAngles.z, "Down");

        return vert | hori1 | hori2 | blocks;
    }

    bool Comp_float(float a, float b, bool ineq = false)
    {
        if (ineq)
        {
    ...
    void Dead_block()
    {
        FindObjectOfType<Master_Control>().AddToBoard(state.position.y, state.position.x, (int)(state.eulerAngles.z), "I");
    ...
    void StateChange()
    {
        float ghoststate = FindObjectOfType<Master_Control>().ChangeGhostPiece("I", state.position.y, state.position.x, (int)state.eulerAngles.z);
